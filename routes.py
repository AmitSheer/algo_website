...
from hypergz import force_directed_hyper_graphs_using_social_and_gravity_scaling, hypergraph, hyperedge, \
    complete_algorithm, cycle_algorithm, star_algorithm, wheel_algorithm
...
def find_algo(algo):
    ...
    if algo == 'Cyc':
        return cycle_algorithm
    if algo == 'Str':
        return star_algorithm
    if algo == 'Wh':
        return wheel_algorithm
...
def home():
    ...
    if request.method == 'POST':
        fig: Figure = force_directed_hyper_graphs_using_social_and_gravity_scaling(
